	...
	var ytEmbed = {
			...
			if(json.error){
				...
				for (var i = 0; i < json.data.items.length; i++) {
					...
					if(this.cfg.layout == 'thumbnails'){
						...
						//this.cfg.layout = full
						li.innerHTML = '<table ><td><h3>'+entry.title+'</h3></td></table>';
				...
				this.message('Kein Video gefunden');
			}
		},
		/**
		...
		*/		
		createPlayer : function(cfg){
			...
			iframe.setAttribute('src', 'http://www.youtube.com/embed/'+ytPlayerParams.videoId+'?autoplay='+ytPlayerParams.autoplay+'&modestbranding=1 allowfullscreen');//controlbar set
